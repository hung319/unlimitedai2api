...
async function* parseUpstreamStream(reader: ReadableStreamDefaultReader<Uint8Array>) {
    ...
    try {
        while (true) {
            ...
            console.log(`🔵 [PARSER DEBUG] Raw chunk: ${decodedChunk.substring(0, 200)}...`);
            ...
            for (const line of lines) {
                console.log(`🔵 [PARSER DEBUG] Processing line: ${line.substring(0, 100)}`);
                ...
                if (key === '0' || key === 'g' || key === '3') {
                    ...
                    if (obj.diff && Array.isArray(obj.diff) && obj.diff.length >= 2) {
                        ...
                        if (obj.next && typeof obj.next === 'string' && obj.next.startsWith('$@')) {
                            const nextKey = obj.next.substring(2);
        ...
        for (const key of contentKeys) {
            ...
            if (obj.diff && Array.isArray(obj.diff) && obj.diff.length >= 2) {
                ...
                if (typeof content === 'string' && content.trim().length > 0) {
                    ...
                    while (objectsMap[currentKey]?.next && chainLength < 20) {
...
async function handleChat(req: Request): Promise<Response> {
    try {
        ...
        if (!validModels.includes(selectedModel)) {
            ...
            selectedModel = "chat-model-reasoning-with-search";
        }
        ...
        const rscPayload = [{
            ...
            deviceId: "796cacff-656b-4e0d-aa17-08810059f1ab"
        }];
